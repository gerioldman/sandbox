...
def getHeaderListFromPreprocessedFileDirectly(file_path):
    headerList = []
    f = open(file_path, "r")
...
# get list of header files from the original source file directly


def getHeaderListFromFileDirectly(file_path) -> list:
    headerList = []
    ...
    for lines in whole_file:
        findlist = re.findall("^#include[ ]+\"(\\w+\\.h)\"", lines)
        if len(findlist) != 0:
            for match in findlist:
                matches.append(match)
    ...
    # remove #include part of the line

    for match in matches:
        match.replace("#include ", "")
        headerList.append(match)
            ...
            type=c_ast.Enum(
                ...
                values=c_ast.EnumeratorList(
                    enumerators=[
                        ...
                        c_ast.Enumerator(
                            name="STUB_OPTION_REDIRECT", value=None)
                    ],
                    coord=None
                )
            ),
        ...
        type=c_ast.TypeDecl(
            ...
            type=c_ast.Enum(
                ...
                values=c_ast.EnumeratorList(
                    enumerators=[
                        c_ast.Enumerator(
                            name="STUB_OPTION_TRACE_OFF", value=None),
                        c_ast.Enumerator(
                            name="STUB_OPTION_TRACE_ON", value=None)
                    ],
                    coord=None
                )
            ),
        ...
        type=c_ast.TypeDecl(
            ...
            type=c_ast.Enum(
                ...
                values=c_ast.EnumeratorList(
                    enumerators=[
                        c_ast.Enumerator(
                            name="STUB_OPTION_PARG_COPY_FROM", value=None),
                        c_ast.Enumerator(
                            name="STUB_OPTION_PARG_COPY_TO", value=None)
                    ],
                    coord=None
                )
            ),
                ...
                decls=[
                    c_ast.Decl(
                        ...
                        type=c_ast.TypeDecl(
                            ...
                            type=c_ast.IdentifierType(
                                names=["stub_trace_option_t"], coord=None),
                            coord=None
                        ),
                        init=None,
                        bitsize=None,
                    ...
                    c_ast.Decl(
                        ...
                        type=c_ast.TypeDecl(
                            ...
                            align=None,
                            type=c_ast.IdentifierType(
                                names=["unsigned", "long"], coord=None),
                            coord=None
                        ),
                        init=None,
                        bitsize=None,
...
def makeStubCompound(entity, test_call):
    ...
    if not test_call:
        if not isVoidType(entity.type.type):
            ...
            blockItems.append(c_ast.Decl(
                ...
                type=c_ast.TypeDecl(
                    ...
                    type=c_ast.IdentifierType(
                        names=entity.type.type.type.names),
                    coord=None,
                    align=entity.type.type.align
                )
            ))
        blockItems.append(c_ast.UnaryOp(op='p++', expr=c_ast.StructRef(name=c_ast.StructRef(name=c_ast.ID(
            name='TEST_STUB'), type='.', field=c_ast.ID(entity.name)), type='.', field=c_ast.ID(name='callcount'))))
        blockItems.append(
            c_ast.Decl(
                name="trace_string",
                quals=[],
        ...
        blockItems.append(
            c_ast.If(
                cond=c_ast.BinaryOp(
                    left=c_ast.BinaryOp(
                        left=c_ast.StructRef(name=c_ast.StructRef(name=c_ast.ID(name='TEST_STUB'), type='.', field=c_ast.ID(
                            "TEST_TRACE")), type='.', field=c_ast.ID(name='trace_enabled')),
                        op="==",
                    ...
                    right=c_ast.BinaryOp(
                        left=c_ast.StructRef(name=c_ast.StructRef(name=c_ast.ID(name='TEST_STUB'), type='.', field=c_ast.ID(
                            "TEST_TRACE")), type='.', field=c_ast.ID(name='trace_buffer')),
                        op="!=",
                        right=c_ast.ID(name="((void *)0)"),
                        coord=None
                    ),
                    coord=None
                ),
                iftrue=c_ast.If(
                    cond=c_ast.BinaryOp(
                        left=c_ast.StructRef(name=c_ast.StructRef(name=c_ast.ID(name='TEST_STUB'), type='.', field=c_ast.ID(
                            "TEST_TRACE")), type='.', field=c_ast.ID(name='trace_buffer_index')),
                        op="<",
                        right=c_ast.StructRef(name=c_ast.StructRef(name=c_ast.ID(name='TEST_STUB'), type='.', field=c_ast.ID(
                            "TEST_TRACE")), type='.', field=c_ast.ID(name='trace_buffer_length')),
                        coord=None
                    ),
                    iftrue=c_ast.Compound(
                        block_items=[
                                ...
                                cond=c_ast.BinaryOp(
                                    ...
                                    left=c_ast.BinaryOp(
                                        ...
                                        right=c_ast.Constant(
                                            type='int', value=len(entity.name) + 1)
                                    ),
                                    right=c_ast.BinaryOp(
                                        op='<',
                                        left=c_ast.ID(name='i'),
                                    ...
                                    block_items=[
                                        c_ast.Assignment(
                                            ...
                                            lvalue=c_ast.ArrayRef(
                                                name=c_ast.ArrayRef(
                                                    name=c_ast.StructRef(name=c_ast.StructRef(name=c_ast.ID(name='TEST_STUB'), type='.', field=c_ast.ID(
                                                        "TEST_TRACE")), type='.', field=c_ast.ID(name='trace_buffer')),
                                                    subscript=c_ast.StructRef(name=c_ast.StructRef(name=c_ast.ID(name='TEST_STUB'), type='.', field=c_ast.ID(
                                                        "TEST_TRACE")), type='.', field=c_ast.ID(name='trace_buffer_index')),
                                                    coord=None
                                                ),
                            ...
                            c_ast.UnaryOp(op='p++', expr=c_ast.StructRef(name=c_ast.StructRef(name=c_ast.ID(name='TEST_STUB'),
                                          type='.', field=c_ast.ID("TEST_TRACE")), type='.', field=c_ast.ID(name='trace_buffer_index')))
                        ],
                        coord=None
                    ),
                    iffalse=None
        ...
        # Adding return value
        if not isVoidType(entity.type.type):
            blockItems[index].stmt.block_items[0].stmts.append(c_ast.Assignment(op='=', lvalue=c_ast.ID(name='returnValue'), rvalue=c_ast.StructRef(
                name=c_ast.StructRef(name=c_ast.ID(name='TEST_STUB'), type='.', field=c_ast.ID(entity.name)), type='.', field=c_ast.ID(name='returnValue'))))
                        ...
                        if param.name != None:
                            if isinstance(param.type, c_ast.TypeDecl):
                                blockItems[index].stmt.block_items[0].stmts.append(
                                    c_ast.Assignment(
                                        op='=',
                                        lvalue=c_ast.StructRef(name=c_ast.StructRef(name=c_ast.ID(name='TEST_STUB'), type='.', field=c_ast.ID(
                                            entity.name)), type='.', field=c_ast.ID(name=param.name)),
                                ...
                                if isinstance(param.type.type, c_ast.FuncDecl):
                                    ...
                                    if param.type.type.type.names == ['void']:
                                        blockItems[index].stmt.block_items[0].stmts.append(
                                            c_ast.Assignment(
                                                op='=',
                                                lvalue=c_ast.StructRef(name=c_ast.StructRef(name=c_ast.ID(
                                                    name='TEST_STUB'), type='.', field=c_ast.ID(entity.name)), type='.', field=c_ast.ID(param.name)),
                                                rvalue=c_ast.ID(
                                                    name=param.name)
                                            )
                                        )
                                        ...
                                        blockItems[index].stmt.block_items[0].stmts.append(
                                            c_ast.Assignment(
                                                ...
                                                rvalue=c_ast.UnaryOp(
                                                    op='*', expr=c_ast.ID(name=param.name))
                                            )
                                        )
                                    else:
                                        blockItems[index].stmt.block_items[0].stmts.append(
                                            c_ast.If(
                                                ...
                                                (
                                                    ...
                                                    left=c_ast.StructRef(
                                                        name=c_ast.StructRef(name=c_ast.StructRef(name=c_ast.ID(name='TEST_STUB'), type='.', field=c_ast.ID(entity.name)), type='.', field=c_ast.ID(
                                                            param.name)),  # c_ast.StructRef( name = c_ast.ID(name='TEST_STUB'), type= '.' , field= c_ast.ID(entity.name))
                                                        type='.',
                                                        field=c_ast.ID(name='stub_pArg_option')),
                                                    right=c_ast.ID(
                                                        name='STUB_OPTION_PARG_COPY_TO'),
                                                    coord=None),
                                                ...
                                                iffalse=c_ast.Compound(block_items=[
                                                    c_ast.If(
                                                        cond=c_ast.BinaryOp(
                                                            ...
                                                            right=c_ast.ID(
                                                                name='STUB_OPTION_PARG_COPY_FROM'),
                                                            coord=None
                                                        ),
                                                        iftrue=c_ast.Compound(
                                                            block_items=[
                                        ...
                                        stmt=c_ast.Compound(
                                            block_items=[
                                                c_ast.Assignment(
                                                    op='=',
                                                    lvalue=c_ast.ArrayRef(
                                                        name=c_ast.StructRef(name=c_ast.StructRef(name=c_ast.ID(
                                                            name='TEST_STUB'), type='.', field=c_ast.ID(entity.name)), type='.', field=c_ast.ID(param.name)),
                                                        subscript=c_ast.ID(
                                                            name='i'),
                                                        coord=None
        ...
        if isVoidType(entity.type.type):
            blockItems[index].stmt.block_items[1].stmts.append(c_ast.FuncCall(name=c_ast.StructRef(name=c_ast.StructRef(name=c_ast.ID(
                name='TEST_STUB'), type='.', field=c_ast.ID(entity.name)), type='.', field=c_ast.ID(name='redirectFuncPtr')), args=c_ast.ExprList(exprs=[])))
        else:
                    ...
                    for param in entity.type.args.params:
                        if param.name != None:
                            if entity.type.type.type.names[0] == 'void':
                                blockItems[index].stmt.block_items[1].stmts[0].args.exprs.append(
                                    c_ast.ID(name=param.name))
                            else:
                                blockItems[index].stmt.block_items[1].stmts[0].right.args.exprs.append(
                                    c_ast.ID(name=param.name))

        if not isVoidType(entity.type.type):
            blockItems.append(c_ast.Return(
                expr=c_ast.ID(name='returnValue'), coord=None))
    ...
    ###############################################
        ...
        blockItems.append(c_ast.UnaryOp(op='p++', expr=c_ast.StructRef(name=c_ast.StructRef(name=c_ast.ID(
            name='TEST_STUB'), type='.', field=c_ast.ID(entity.name)), type='.', field=c_ast.ID(name='callcount'))))
        if isVoidType(entity.type.type):
            blockItems.append(c_ast.FuncCall(name=c_ast.ID(
                name=entity.name), args=c_ast.ExprList(exprs=[])))
        else:
            blockItems.append(
                c_ast.Return(

                    c_ast.FuncCall(name=c_ast.ID(name=entity.name),
                                   args=c_ast.ExprList(exprs=[]))
                )
            )
        if entity.type.args != None:
            if hasattr(entity.type.args, 'params'):
                if entity.type.args.params != None:
                    for param in entity.type.args.params:
                        if param.name != None:
                            if entity.type.type.type.names[0] == 'void':
                                blockItems[1].args.exprs.append(
                                    c_ast.ID(name=param.name))
                            else:
                                blockItems[1].expr.args.exprs.append(
                                    c_ast.ID(name=param.name))
    return blockItems
    ...
    for entity in funcDeclList:
        if isinstance(entity.type, c_ast.FuncDecl):
            funcDef = c_ast.FuncDef(
                decl=entity, param_decls=None, body=c_ast.Compound(block_items=[]))
...
*\n\
*/\n")
    mock_internal_header = sys.argv[5].split("/")[-1]
    mock_header = sys.argv[4].split("/")[-1]

    f.write("#include \"" + mock_internal_header + "\"\n")
    f.write("#include \"" + mock_header + "\"\n")
    f.write(generator.visit(local_ast))
    f.close()

# create the required header file
# DO NOT USE THIS FUNCTION TO CREATE types.h
...
def main():
    if len(sys.argv) > 2:
        # try:
        ast = parse_file(sys.argv[2], use_cpp=False)
        # except:
        ...
        headerList = getHeaderListFromPreprocessedFileDirectly(sys.argv[2])
        headerList = getHeaderListFromFileDirectly(sys.argv[6])
...
#
        ...
        f = open(sys.argv[4], "w")
        f.write("#ifndef MOCK_H\n")
        f.write("#define MOCK_H\n")
        f.write("#include \"" + sys.argv[5].split("/")[-1] + "\"\n")
        funcDefList = getListofFuncDef(ast)
